    ...
    fn produces_declared_p3_p4_p5_shapes() {
        let worker = std::thread::Builder::new()
            ...
            .spawn(|| {
                ...
                let body: Yolo11BodySmall<Flex> = Yolo11BodyNConfig.init(&device);
                ...
                let body: Yolo11BodySmall<Flex> = Yolo11BodySConfig.init(&device);
                let output = body.forward(input.clone());
                assert_eq!(output.p3.dims(), [1, 128, 8, 8]);
                assert_eq!(output.p4.dims(), [1, 256, 4, 4]);
                assert_eq!(output.p5.dims(), [1, 512, 2, 2]);

                let body: Yolo11BodyLarge<Flex> = Yolo11BodyMConfig.init(&device);
                let output = body.forward(input.clone());
                assert_eq!(output.p3.dims(), [1, 256, 8, 8]);
                assert_eq!(output.p4.dims(), [1, 512, 4, 4]);
                assert_eq!(output.p5.dims(), [1, 512, 2, 2]);

                let body: Yolo11BodyLarge<Flex> = Yolo11BodyLConfig.init(&device);
                let output = body.forward(input.clone());
                assert_eq!(output.p3.dims(), [1, 256, 8, 8]);
                assert_eq!(output.p4.dims(), [1, 512, 4, 4]);
                assert_eq!(output.p5.dims(), [1, 512, 2, 2]);

                let body: Yolo11BodyLarge<Flex> = Yolo11BodyXConfig.init(&device);
