    ...
    fn produces_declared_p3_p4_p5_shapes() {
        let worker = std::thread::Builder::new()
            ...
            .spawn(|| {
                ...
                let body: Yolov10BodyN<Flex> = Yolov10BodyNConfig.init(&device);
                ...
                let body: Yolov10BodyS<Flex> = Yolov10BodySConfig.init(&device);
                let output = body.forward(input.clone());
                assert_eq!(output.p3.dims(), [1, 128, 8, 8]);
                assert_eq!(output.p4.dims(), [1, 256, 4, 4]);
                assert_eq!(output.p5.dims(), [1, 512, 2, 2]);

                let body: Yolov10BodyM<Flex> = Yolov10BodyMConfig.init(&device);
                let output = body.forward(input.clone());
                assert_eq!(output.p3.dims(), [1, 192, 8, 8]);
                assert_eq!(output.p4.dims(), [1, 384, 4, 4]);
                assert_eq!(output.p5.dims(), [1, 576, 2, 2]);

                let body: Yolov10BodyB<Flex> = Yolov10BodyBConfig.init(&device);
                let output = body.forward(input.clone());
                assert_eq!(output.p3.dims(), [1, 256, 8, 8]);
                assert_eq!(output.p4.dims(), [1, 512, 4, 4]);
                assert_eq!(output.p5.dims(), [1, 512, 2, 2]);

                let body: Yolov10BodyB<Flex> = Yolov10BodyLConfig.init(&device);
                let output = body.forward(input.clone());
                assert_eq!(output.p3.dims(), [1, 256, 8, 8]);
                assert_eq!(output.p4.dims(), [1, 512, 4, 4]);
                assert_eq!(output.p5.dims(), [1, 512, 2, 2]);

                let body: Yolov10BodyX<Flex> = Yolov10BodyXConfig.init(&device);
                let output = body.forward(input);
                assert_eq!(output.p3.dims(), [1, 320, 8, 8]);
                assert_eq!(output.p4.dims(), [1, 640, 4, 4]);
                assert_eq!(output.p5.dims(), [1, 640, 2, 2]);
            })
