...
impl Yolo26BodyNConfig {
    pub fn init<B: Backend>(&self, device: &Device<B>) -> Yolo26BodySmall<B> {
        Yolo26BodySmall {
            ...
            model_19: C3k2C3kConfig::new(192, 128, 1, true, 0.5).init(device),
...
/// Configuration for the fixed YOLO26s body (depth 0.50, width 0.50, max channels 1024).
#[derive(Debug, Default)]
...
impl Yolo26BodySConfig {
    pub fn init<B: Backend>(&self, device: &Device<B>) -> Yolo26BodySmall<B> {
        Yolo26BodySmall {
            ...
            model_19: C3k2C3kConfig::new(384, 256, 1, true, 0.5).init(device),
            model_20: ConvConfig::new(256, 256, 3, 2).init(device),
            model_22: C3k2AttnConfig::new(768, 512, 1, true).init(device),
        }
    }
}

/// Complete YOLO26 backbone and feature-pyramid body (layers 0-22), m/l/x scales.
///
...
/// Configuration for the fixed YOLO26m body (depth 0.50, width 1.00, max channels 512).
#[derive(Debug, Default)]
...
impl Yolo26BodyMConfig {
    pub fn init<B: Backend>(&self, device: &Device<B>) -> Yolo26BodyLarge<B> {
        Yolo26BodyLarge {
            ...
            model_22: C3k2AttnConfig::new(1024, 512, 1, true).init(device),
        }
    }
}

/// Configuration for the fixed YOLO26l body (depth 1.00, width 1.00, max channels 512). The
/// attention stage at layer 22 keeps its declared repeat of one.
#[derive(Debug, Default)]
...
/// Configuration for the fixed YOLO26x body (depth 1.00, width 1.50, max channels 512).
#[derive(Debug, Default)]
...
impl Yolo26BodyXConfig {
    pub fn init<B: Backend>(&self, device: &Device<B>) -> Yolo26BodyLarge<B> {
        Yolo26BodyLarge {
            ...
            model_22: C3k2AttnConfig::new(1536, 768, 1, true).init(device),
        }
    }
}
    ...
    fn produces_declared_p3_p4_p5_shapes() {
        let worker = std::thread::Builder::new()
            ...
            .spawn(|| {
                ...
                let body: Yolo26BodySmall<Flex> = Yolo26BodyNConfig.init(&device);
                ...
                let body: Yolo26BodySmall<Flex> = Yolo26BodySConfig.init(&device);
                let output = body.forward(input.clone());
                assert_eq!(output.p3.dims(), [1, 128, 8, 8]);
                assert_eq!(output.p4.dims(), [1, 256, 4, 4]);
                assert_eq!(output.p5.dims(), [1, 512, 2, 2]);

                let body: Yolo26BodyLarge<Flex> = Yolo26BodyMConfig.init(&device);
                let output = body.forward(input.clone());
                assert_eq!(output.p3.dims(), [1, 256, 8, 8]);
                assert_eq!(output.p4.dims(), [1, 512, 4, 4]);
                assert_eq!(output.p5.dims(), [1, 512, 2, 2]);

                let body: Yolo26BodyLarge<Flex> = Yolo26BodyLConfig.init(&device);
                let output = body.forward(input.clone());
                assert_eq!(output.p3.dims(), [1, 256, 8, 8]);
                assert_eq!(output.p4.dims(), [1, 512, 4, 4]);
                assert_eq!(output.p5.dims(), [1, 512, 2, 2]);

                let body: Yolo26BodyLarge<Flex> = Yolo26BodyXConfig.init(&device);
