...
use super::{
    body::{Yolo26Body, Yolo26BodyConfig},
...
impl Yolo26Config {
    pub fn init<B: Backend>(&self, device: &Device<B>) -> Yolo26<B> {
        Yolo26 {
            body: Yolo26BodyConfig.init(device),
            head: Yolo26HeadConfig.init(device),
        }
    }
}
...
mod tests {
    ...
    fn imports_official_checkpoint_and_runs_forward() {
        let checkpoint = std::path::PathBuf::from("target/yolo26n-state.pt");
        assert!(
            checkpoint.exists(),
            "convert yolo26n.pt with tools/export_ultralytics_state.py first"
        );
    ...
    fn matches_ultralytics_golden_tensors() {
        let checkpoint =
            std::path::PathBuf::from("target/yolo26n-coco-ultralytics-v8.4-boquilens-v1.bpk");
        let fixture: GoldenFixture = serde_json::from_slice(
            &std::fs::read("target/yolo26n-golden-v1.json")
                .expect("generate fixtures with tools/export_yolo26_fixtures.py"),
        )
        .unwrap();
        assert_eq!(fixture.format, "boquilens-ultralytics-golden-v1");
        assert_eq!(fixture.model, "yolo26n");
        ...
        let worker = std::thread::Builder::new()
            ...
            .spawn(move || {
                ...
                let image = image::open("target/yolo26n-preprocessed-reference.png")
                    .unwrap()
                    .into_rgb8();
                ...
                let raw = model.head.forward_raw(super::super::body::Yolo26Features {
