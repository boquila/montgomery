...
use super::body::Yolo26Features;

const DEFAULT_NUM_CLASSES: usize = 80;

/// YOLO26 declares `reg_max: 1`, so each box side is a direct distance and no DFL projection
/// exists.
const REG_MAX: usize = 1;

/// Ultralytics Detect inference parameters.
pub const MAX_DETECTIONS: usize = 300;

/// Raw one2one predictions before anchor-grid decoding.
...
/// One detection scale of the YOLO26 one2one head.
///
/// The box tower matches Ultralytics' shared `cv2` layout; the light classification tower matches
/// `Detect.cv3`: depth-wise/pointwise pairs followed by a biased 1x1 projection. Field names
...
/// Ultralytics YOLO26 `Detect` head, one2one (NMS-free) end-to-end inference branch.
///
/// YOLO26 is DFL-free with `reg_max = 1`: the box tower emits the four XYXY side distances
/// directly. Training builds also carry the official one-to-many branch; inference selects the
/// one-to-one predictions by top score without non-maximum suppression.
#[derive(Module, Debug)]
pub struct Yolo26Head {
    p3: DetectionBranch,
    p4: DetectionBranch,
    p5: DetectionBranch,
    #[cfg(feature = "training")]
...
impl Yolo26Head {
    pub fn forward_raw(&self, features: Yolo26Features) -> RawPredictions {
    ...
    #[cfg(feature = "training")]
    pub fn forward_dual(&self, features: Yolo26Features) -> DualRawPredictions {
        let Yolo26Features { p3, p4, p5 } = features;
    ...
    pub fn forward(&self, features: Yolo26Features) -> DecodedPredictions {
        ...
        // End-to-end decoding is XYXY: the first two distances are the top-left offsets from the
        // anchor and the last two the bottom-right offsets, scaled to model-input pixels.
        let distances = raw.boxes.swap_dims(1, 2);
...
impl Yolo26HeadConfig {
    ...
    pub fn init(&self, device: &Device) -> Yolo26Head {
    ...
    fn decodes_three_feature_levels_to_xyxy_and_scores() {
        let worker = std::thread::Builder::new()
            ...
            .spawn(|| {
                ...
                let body = Yolo26BodyNConfig.init(&device);
                let head = Yolo26HeadConfig::new(64, 128, 256).init(&device);
