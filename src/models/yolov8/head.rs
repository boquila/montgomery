...
/// YOLOv8 keeps Ultralytics' classic DFL head: each box side is a 16-bin distribution projected
...
///
/// The box tower matches Ultralytics' shared `cv2` layout. The classification tower is the
/// **legacy** full-3x3-conv `cv3` flavor: YOLOv8 checkpoints were trained before the light
/// DWConv/Conv tower existed (the C2f body keeps `Detect.legacy` semantics of the v8 era), and the
...
impl<B: Backend> DetectionBranch<B> {
    fn forward(&self, input: Tensor<B, 4>) -> (Tensor<B, 3>, Tensor<B, 3>) {
        ...
        let cls = self.cls_1.forward(self.cls_0.forward(input));
        let scores = self
            .cls_out
            .forward(cls)
    ...
    fn init<B: Backend>(&self, device: &Device<B>) -> DetectionBranch<B> {
        DetectionBranch {
            ...
            box_out: Conv2dConfig::new([self.box_channels, 4 * REG_MAX], [1, 1])
                .with_bias(true)
                .init(device),
            cls_0: ConvConfig::new(self.input_channels, self.cls_channels, 3, 1).init(device),
            cls_1: ConvConfig::new(self.cls_channels, self.cls_channels, 3, 1).init(device),
...
/// Ultralytics YOLOv8 `Detect` head, classic one2many inference branch.
///
/// Like YOLO11 this head is not end-to-end: its predictions are plain per-anchor distributions
...
impl Yolov8HeadConfig {
    /// Declare the head for one scale from its P3/P4/P5 input widths.
    ///
    /// `Detect` derives the box tower width as `max(16, ch[0] / 4, reg_max * 4)` and the legacy
    /// classification tower width as `max(ch[0], min(nc, 100))`; with `reg_max = 16` and
    /// `nc = 80` the box width is 64 at n/s/m/l scale and 80 at x scale (ch[0] = 320), and the
