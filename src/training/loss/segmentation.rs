...
pub fn bilinear_upsample_2x(input: Tensor<4>) -> Tensor<4> {
...
pub fn instance_mask_loss(
    ...
    prototypes: Tensor<4>,
    target_masks: Tensor<4>,
    matches: &[MaskMatch],
) -> Result<Tensor<1>, &'static str> {
    ...
    let coefficient_indices =
        Tensor::<1, Int>::from_data(TensorData::new(coefficient_indices, [slot_count]), &device);
    let target_indices =
        Tensor::<1, Int>::from_data(TensorData::new(target_indices, [slot_count]), &device);
    ...
    let targets = target_masks
        ...
        .unsqueeze_dim::<4>(1);
    let crop = Tensor::<4>::from_data(
        TensorData::new(crop, [slot_count, 1, height, width]),
        &device,
    );
    let normalizers =
        Tensor::<2>::from_data(TensorData::new(normalizers, [matched_count, 1]), &device);
    let per_match = (bce_with_logits_tensor(logits, targets) * crop)
        .reshape([slot_count, height * width])
        .sum_dim(1)
        .select(
            0,
            Tensor::<1, Int>::from_data(TensorData::new(valid_indices, [matched_count]), &device),
        );
...
pub fn semantic_bce_dice_loss(
    logits: Tensor<4>,
    class_map: Tensor<3, Int>,
    coverage: Tensor<3, Bool>,
) -> Result<Tensor<1>, &'static str> {
    ...
    let target = if classes == 1 {
        ...
        let one_hot: Tensor<4, Int> = class_map.one_hot(classes);
...
pub fn dual_semantic_bce_dice_loss(
    logits: Tensor<4>,
    class_map: Tensor<3, Int>,
    coverage: Tensor<3, Bool>,
) -> Result<(Tensor<1>, Tensor<1>), &'static str> {
...
mod tests {
    ...
    fn dual_semantic_loss_reuses_value_but_detaches_one_to_one() {
        ...
        let class_map = Tensor::<3, Int>::from_ints([[[0, 0], [0, 0]]], &device);
        let coverage = Tensor::<3, Bool>::from_bool([[[true, false], [true, false]]], &device);
    ...
    fn batched_instance_masks_match_scalar_reference_and_backpropagate() {
        ...
        let coefficients = Tensor::<3>::from_data(
            TensorData::new(coefficients_data.clone(), [2, 2, 3]),
            &device,
        )
        .require_grad();
        let prototypes = Tensor::<4>::from_data(
            TensorData::new(prototypes_data.clone(), [2, 2, 2, 2]),
            &device,
        )
        .require_grad();
        let targets =
            Tensor::<4>::from_data(TensorData::new(targets_data.clone(), [2, 2, 2, 2]), &device);
