"""Export reproducible YOLO26-seg preprocessing and tensor parity fixtures.

Covers every segment scale (n/s/m/l/x): pass ``--model yolo26n-seg`` with the matching official
checkpoint. The generated files belong in ``target/`` and are intentionally not committed: they
are derived from an external Ultralytics checkpoint. The Rust ignored tests consume them to
distinguish image preprocessing drift from model-graph drift.
"""
...
def main() -> None:
    ...
    parser.add_argument("--model", default="yolo26n-seg", help="segment scale being exported")
    args = parser.parse_args()
    if not args.model.startswith("yolo26") or not args.model.endswith("-seg"):
        raise ValueError(f"{args.model} is not a YOLO26 segment scale")
    ...
    with torch.inference_mode():
        (y, proto), preds = model(input_tensor)
    for hook in hooks:
        hook.remove()
    ...
    tensors = {
        **captured,
        ...
        "mask_coeffs": one2one["mask_coefficient"],
        "protos": proto,
    }
    fixture = {
