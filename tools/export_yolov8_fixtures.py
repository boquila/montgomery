"""Export reproducible YOLOv8 preprocessing and tensor parity fixtures.

Covers every detect scale (n/s/m/l/x) and the segmentation variants (``yolov8n-seg`` ..
``yolov8x-seg``): pass ``--model yolov8s`` with the matching checkpoint. The generated files
...
def main() -> None:
    ...
    parser.add_argument("--model", default="yolov8n", help="detect or seg scale being exported")
    args = parser.parse_args()
    if not args.model.startswith("yolov8"):
        raise ValueError(f"{args.model} is not a YOLOv8 detect/seg scale")
    ...
    hooks = [
        model.model[15].register_forward_hook(capture("body_p3")),
        model.model[18].register_forward_hook(capture("body_p4")),
        model.model[21].register_forward_hook(capture("body_p5")),
    ]
    ...
    # The classic YOLOv8 head concatenates DFL-decoded center-size boxes with sigmoid scores into
